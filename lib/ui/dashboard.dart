...
class User extends StatefulWidget {
  ...
  //User(key, {required Key key}) : super(key: key);
...
class _UserState extends State<User> {
  ...
  static GoogleMapController _mapController=0 as GoogleMapController;
  Location _locationTracker = Location();

  Map<String, Marker> _allMarkers = new Map();
  Map<String, DriverData> _drivers= new Map();

  late StreamSubscription<QuerySnapshot> _subscribe;

  late DriverData? currentDriver;
  bool _showCurrentDriver = false;

  late DriverData? _currentSOS;
  ...
  void _sos(Map<String, DriverData> drivers) async {
    ...
    await Future.forEach(keyList, (key) async {
     ...
     double distanceInMeters = await Geolocator.distanceBetween(
          ...
          d?.latlong.latitude??0,
          d?.latlong.longitude??0);
  ...
  RaisedButton({required RoundedRectangleBorder shape, required Null Function() onPressed, required MaterialColor color, required Row child}) {}

  FlatButton({required RoundedRectangleBorder shape, required Text child, required Null Function() onPressed, required MaterialColor color}) {}
}
